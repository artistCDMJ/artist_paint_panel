...
class ArtistPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator("import_image.to_plane", text = "Image to plane", icon = 'IMAGE_COL')
        
        row = layout.row()
        row.operator("image.reload_saved_state", text = "Reload Image", icon = 'LOAD_FACTORY')
        
        row = layout.row()
        row.operator("image.save_current", text = "Save Image", icon = 'IMAGEFILE')
        ...
        row = layout.row()
        row.operator("image.cameraview_paint", text = "Camera View Paint", icon = 'RENDER_REGION')
